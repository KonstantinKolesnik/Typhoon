...
unsigned char copy[] PROGMEM = {"..SignalDecoder V0.11 .."};
...
void action(void)
  {
    ...
    if (MyOpMode == 7)
      ...
      {                            // new for alex shepherd - uses all 16 commands
        set_new_light_val(
            eeprom_read_byte(&EE_Signal_comb_Pattern[myCommand]),
            eeprom_read_byte(&EE_Signal_comb_Valid[myCommand])    );
      }
    else if (MyOpMode == 5)
      {
        set_new_light_val(
            eeprom_read_byte(&EE_Signal_bin_Pattern[myCommand]),
            eeprom_read_byte(&EE_Signal_bin_Valid[myCommand])    );
      }
    else if (MyOpMode == 4)
      ...
      {   											// 3: Einfahrsignal + Vorsignal mit Dunkeltastung
            ...
            if ((hauptsignal_state == 0)||(hauptsignal_state == 3)) 
              {
                ...
                set_new_light_val(
                    eeprom_read_byte(&EE_Signal_DB4_Pattern[vorsignal_state]),
                    eeprom_read_byte(&EE_Signal_DB4_Valid[vorsignal_state])    );  
              }
            hauptsignal_state = myCommand;
            set_new_light_val(
                eeprom_read_byte(&EE_Signal_DB4_Pattern[hauptsignal_state]),
                eeprom_read_byte(&EE_Signal_DB4_Valid[hauptsignal_state])    ); 
          }
        else if (myCommand <= 7)
          {
        ...
        if ((hauptsignal_state == 0)||(hauptsignal_state == 3)) 
          {                                        // mit Dunkeltastung
            ...
            set_new_light_val(
                eeprom_read_byte(&EE_Signal_DB4_Pattern[7]),
                eeprom_read_byte(&EE_Signal_DB4_Valid[7])    );
          }
      }
    else if (MyOpMode == 2)         
      {                             // Einfahrsignal + Vorsignal getrennt
        if (myCommand < 8)
          {
            set_new_light_val(
                eeprom_read_byte(&EE_Signal_DB4_Pattern[myCommand]),
                eeprom_read_byte(&EE_Signal_DB4_Valid[myCommand])    );
          }
      }
    else if (MyOpMode == 1)        
      {                             // Zwei dreibegriffige Signale (DB)
        if (myCommand < 8)
          {
            set_new_light_val(
                eeprom_read_byte(&EE_Signal_DB3_Pattern[myCommand]),
                eeprom_read_byte(&EE_Signal_DB3_Valid[myCommand])    );
          }
      }
    else
      {   // MyOpMode == 0          // Vier zweibegriffige Signale
        if (myCommand < 8)
          {
            set_new_light_val(
                eeprom_read_byte(&EE_Signal_DB2_Pattern[myCommand]),
                eeprom_read_byte(&EE_Signal_DB2_Valid[myCommand])    );
          }
      }  // OpMode
    sei();
  }
...
int main(void)
  {
    ...
    CurrentTarget = eeprom_read_byte(&EE_LastState);
        ...
        if (Communicate & (1<<C_DoSave) )
          {
            ...
            if (JUMPER_FITTED)
              {
                eeprom_write_byte(&EE_LastState, CurrentTarget);   
              } 
          } 
        sei();
        sei();
