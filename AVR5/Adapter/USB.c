...
typedef struct
{
   uchar Type;
   uchar Data[10];
} Packet; // ќписание структуры дл€ передачи данных, 11 bytes + 1 byte is for packet id (=0)
...
void PrepareAnswer()
{
	...
	packet.Data[4] = DCCOutQueryRailcom();
...
inline void ParseCommand()
{
	...
	{
		case 'D': // DCC command
			msg.Repeats = packet.Data[0];
			msg.Size = packet.Data[1];
			memcpy(msg.Dcc, &packet.Data[2], msg.Size);
			if (PROG_IS_ON)
