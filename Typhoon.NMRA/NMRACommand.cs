...
{
    public enum NMRACommandPriority
    {
        Normal,
        High
    }

    public sealed class NMRACommand
    {
        #region Fields
        private NMRACommandPriority priority = NMRACommandPriority.Normal;
        private NMRACommandType type = NMRACommandType.None;
        private int repeats = 0;
        private List<byte> data = new List<byte>(); // just instruction bytes
        #endregion

        #region Properties
        public NMRACommandPriority Priority
        {
            get { return priority; }
            set { priority = value; }
        }
        public NMRACommandType Type
        {
            get { return type; }
            set { type = value; }
        }
        ...
        #region Constructors
        private NMRACommand(List<byte> dataBytes)
        {
            if (dataBytes != null)
                data.AddRange(dataBytes);
        }
        private NMRACommand(NMRACommandPriority priority, List<byte> dataBytes)
            : this(dataBytes)
        {
            this.priority = priority;
        }
        private NMRACommand(NMRACommandType type, List<byte> dataBytes)
            : this(dataBytes)
        {
            this.type = type;
        }
        private NMRACommand(NMRACommandPriority priority, NMRACommandType type, List<byte> dataBytes)
            : this(dataBytes)
        {
            this.priority = priority;
            this.type = type;
        ...
        private NMRACommand(byte[] dataBytes)
        {
            if (dataBytes != null)
                data.AddRange(dataBytes);
        }
        private NMRACommand(NMRACommandPriority priority, byte[] dataBytes)
            : this(dataBytes)
        {
            this.priority = priority;
        }
        ...
        /*
        public static bool operator == (NMRACommand cmd1, NMRACommand cmd2)
        {
        ...
        {
            return (obj is NMRACommand ? AreEqual(this, obj as NMRACommand) : false);
        }
        */
        #endregion

        #region Private methods
        //private static bool AreEqual(NMRACommand cmd1, NMRACommand cmd2)
        //{
        ...
        */
        private static List<byte> BuildLocoAddressBytes(uint address, bool longAddress)
        {
            ...
            {
                if (address >= 1 && address <= NMRA.LocoShortAddressMax)
        ...
        public static NMRACommand Idle()
        {
            List<byte> list = new List<byte>();
            list.Add(NMRA.IdleAddress);
            list.Add(0);

            return new NMRACommand(list);
        }

        #region Locomotives
        private static NMRACommand LocoBroadcast(NMRACommandPriority priority, NMRACommandType type, byte[] instruction)
        {
            List<byte> list = new List<byte>();
            list.Add(NMRA.LocoBroadcastAddress);
            list.AddRange(instruction);

            return new NMRACommand(priority, type, list);
        }
        private static NMRACommand LocoBroadcastBrakeStop(bool forward, bool ignoreDirection, bool emergencyStop)
        {
            //01DC000S

            byte instruction = (byte)(
                (forward ? NMRA.SpeedForward : NMRA.SpeedReverse) | // 01D00000
                (ignoreDirection ? 0x10 : 0x00) | // 000C0000
                (emergencyStop ? 0x01 : 0x00) // 0000000S
                );

            return LocoBroadcast(NMRACommandPriority.High, NMRACommandType.Stop, new byte[] { instruction });
        }
        
        public static NMRACommand LocoBroadcastReset()
        {
            /*
        ...
        public static NMRACommand LocoBroadcastBrake()
        {
            return LocoBroadcastBrakeStop(true, true, false);
        }
        public static NMRACommand LocoBroadcastStop()
        {
        ...
        public static NMRACommand LocoSpeed14(LocomotiveAddress address, int speed, bool forward, bool FL)
        {
            // 01DCSSSS
        ...
        15  1111    14
            */
            ...
            list.Add((byte)(
                ...
                (forward ? NMRA.SpeedForward : NMRA.SpeedReverse) // 01DCSSSS
                ));
        ...
        public static NMRACommand LocoSpeed28(LocomotiveAddress address, int speed, bool forward)
        {
            // 01DCSSSS
        ...
        public static NMRACommand LocoSpeed128(LocomotiveAddress address, int speed, bool forward)
        {
       ...
       127  1111111 126
            */
            ...
            List<byte> list = new List<byte>();
            list.AddRange(BuildLocoAddressBytes(address));
            list.Add((byte)(NMRA.AdvancedOperation | 0x1F)); // 00111111
            list.Add((byte)(
        ...
        #region Functions
        public static NMRACommand LocoFunctionGroup1(LocomotiveAddress address, bool F0, bool F1, bool F2, bool F3, bool F4)
        {
            // 100CFFFF
            ...
            return new NMRACommand(NMRACommandPriority.Normal, NMRACommandType.Function, list);
        }
        public static NMRACommand LocoFunctionGroup2(LocomotiveAddress address, bool F5, bool F6, bool F7, bool F8)
        {
            // 101SFFFF
            ...
            list.AddRange(BuildLocoAddressBytes(address));
            list.Add((byte)(
                NMRA.FunctionGroup2 | 0x10 | // 10110000
                (F5 ? 0x01 : 0) |
                (F6 ? 0x02 : 0) |
                (F7 ? 0x04 : 0) |
                (F8 ? 0x08 : 0)
        ...
        public static NMRACommand LocoFunctionGroup3(LocomotiveAddress address, bool F9, bool F10, bool F11, bool F12)
        {
            // 101SFFFF
        ...
        public static NMRACommand LocoFunctionGroup4(LocomotiveAddress address, bool F13, bool F14, bool F15, bool F16, bool F17, bool F18, bool F19, bool F20)
        {
            // 11011110 FFFFFFFF
            ...
            list.AddRange(BuildLocoAddressBytes(address));
            list.Add((byte)(
                NMRA.Reserved | 0x1E | // 11000000 | 00011110
                (F13 ? 0x01 : 0) |
                (F14 ? 0x02 : 0) |
                (F15 ? 0x04 : 0) |
                (F16 ? 0x08 : 0) |
        ...
        public static NMRACommand LocoFunctionGroup5(LocomotiveAddress address, bool F21, bool F22, bool F23, bool F24, bool F25, bool F26, bool F27, bool F28)
        {
            // 11011111 FFFFFFFF
            ...
            list.AddRange(BuildLocoAddressBytes(address));
            list.Add((byte)(
                NMRA.Reserved | 0x1F | // 11000000 | 00011111
                (F21 ? 0x01 : 0) |
                (F22 ? 0x02 : 0) |
                (F23 ? 0x04 : 0) |
                (F24 ? 0x08 : 0) |
        ...
        public static NMRACommand LocoConsist(LocomotiveAddress address, byte consistAddress, bool forward)
        {
            // 0001CCCC 0AAAAAAA
            ...
            list.AddRange(BuildLocoAddressBytes(address));
            list.Add((byte)(
                NMRA.ConsistControl | // 00010000
                (forward ? 0x02 : 0x03)
                ));
            list.Add((byte)(consistAddress & 0x7F));

            return new NMRACommand(NMRACommandPriority.Normal, NMRACommandType.None, list);
        }
        ...
        #region Service mode
        public static NMRACommand progReset()
        {
            NMRACommand cmd = NMRACommand.LocoBroadcastReset();
            cmd.Priority = NMRACommandPriority.Normal;
            cmd.Type = NMRACommandType.Service;
            return cmd;
        }
        #region Direct mode
        ...
        public static NMRACommand progPagePreset()
        {
            // 01111101 00000001
        ...
        public static NMRACommand progRegisterMode(uint cv, byte value, bool verify = false)
        {
            // 0111CRRR DDDDDDDD
        ...
        public static NMRACommand progAddressOnlyMode(byte value, bool verify = false)
        {
            // 0111C000 0DDDDDDD
            //     C=0 Verify address, 0x00
            //     C=1 Write address, 0x08
        ...
        #region Accessories
        public static NMRACommand BasicAccessory(uint address, char output, bool on)//unsigned int nr, char output, char activate
        {
            // duration of time each output is active being controlled by CVs #515...518
            // 10AAAAAA 1AAACDDD
            //              C = 0 => off
            //              C = 1 => on
            ...
            // Notes:   10111111 1000BCCC is broadcast for basic accessory decoders;
            //          aaa is bit 7...9 of address, but is transmitted inverted!

            // parameters: address: turnout [0000-4095]
            //             output: coil (red, green) [0,1]
            //             on: on off, [0,1]; note: intellibox only sends on, never off :-o
            //
           ...
           // new_message->dcc[1] |= ((on & 0x01) * 0x08);   // add B
           // new_message->dcc[1] |= (pairnr * 2) | (output & 0x01);
            ...
            return new NMRACommand(NMRACommandPriority.Normal, NMRACommandType.Accessory, list);
        }
