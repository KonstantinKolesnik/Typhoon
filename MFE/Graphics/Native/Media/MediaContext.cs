...
namespace MFE.Graphics.Native.Media
{
    ...
    /// </summary>
    internal class MediaContext : IDisposable
    {
        private class GlobalLock { }
        private class InvokeOnRenderCallback
        {
            private MfeDispatcherOperationCallback _callback;
            private object _arg;

            public InvokeOnRenderCallback(MfeDispatcherOperationCallback callback, object arg)
            {
                _callback = callback;
                _arg = arg;
            }
        ...
        private MfeDispatcherOperation _currentRenderOp;
        private MfeDispatcherOperationCallback _renderMessage;
        ...
        private int _screenW = LCDManager.ScreenWidth, _screenH = LCDManager.ScreenHeight;
        private int _dirtyX0 = LCDManager.ScreenWidth, _dirtyY0 = LCDManager.ScreenHeight, _dirtyX1 = 0, _dirtyY1 = 0;
        ...
        {
            _renderMessage = new MfeDispatcherOperationCallback(RenderMessageHandler);

            // we have one render target, the window manager
            _target = WindowManager.Instance;
            _screen = new Bitmap(LCDManager.ScreenWidth, LCDManager.ScreenHeight);
        }
        ...
        //    MfeMediaContext cm = dispatcher._mediaContext;
        ...
        internal void BeginInvokeOnRender(MfeDispatcherOperationCallback callback, object arg)
        {
            ...
            try
            {
                ...
                try
                {
                    if (w > 0 && h > 0)
                    {
                        ...
                        //
                        // We've now updated layout and the updated scene will be rendered.
