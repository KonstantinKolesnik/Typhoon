...
namespace MFE.Graphics.Presentation
{
/*
    /// <summary>+
...
*/
    ...
    public class MfeApplication : MfeDispatcherObject, IEventListener
    {
        #region Private Fields
        private class GlobalLock { }
        static private bool _isShuttingDown;
        static private bool _appCreatedInThisAppDomain;
        static private MfeApplication _appInstance;

        private MfeWindowCollection _appWindowList;
        private MfeWindowCollection _nonAppWindowList;
        private MfeWindow _mainWindow;
        ...
        private static MfeDispatcherOperationCallback _reportInputMethod;
        private static InputManager _inputManager = null;
        private InputProviderSite _inputProviderSite = null;

        private static int _stylusMaxX = LCDManager.ScreenWidth;
        private static int _stylusMaxY = LCDManager.ScreenHeight;
        ...
        */
        ...
        /// </summary>
        static public MfeApplication Current
        {
            get
            {
        ...
        public MfeWindowCollection Windows
        {
            get { return WindowsInternal.Clone(); }
        }
        /// <summary>
        ...
        public MfeApplication()
        {
            ...
            MfeDispatcher.SetFinalDispatcherExceptionHandler(new MfeDispatcherExceptionEventHandler(DefaultContextExceptionHandler));
            ...
            Dispatcher.BeginInvoke(new MfeDispatcherOperationCallback(StartupCallback), null);
        }
        ...
        public void InitializeForEventSource()
        {
            if (_inputManager == null)
            {
                ...
                _inputProviderSite = _inputManager.RegisterInputProvider(this);
                _reportInputMethod = new MfeDispatcherOperationCallback(delegate(object o)
                    {
        ...
        public bool OnEvent(BaseEvent ev)
        {
            ...
            if (touchEvent != null)
            {
                ...
                if (targetWindow != null && touchEvent.EventMessage == (byte)TouchMessages.Down)
                {
                    ...
                    if (!(x <= xSrc && xSrc <= (x + w) && y <= ySrc && ySrc <= (y + h)))
                    {
                        ...
                        if (xSrc <= LCDManager.ScreenWidth && ySrc <= LCDManager.ScreenHeight)
                            targetWindow = null;
                    }
                }

                //If we can enforce that the first event in the array is always the primary touch, we don't have to search.
                if (targetWindow == null)
                    targetWindow = MfeWindowManager.Instance.GetPointerTarget(touchEvent.Touches[0].X, touchEvent.Touches[0].Y);
                
                _inputManager.TouchDevice.SetTarget(targetWindow != null ? targetWindow : MainWindow);

                ir = new RawTouchInputReport(null, touchEvent.Time, touchEvent.EventMessage, touchEvent.Touches);
                dev = _inputManager.TouchDevice;
            }
            else if (ev is GenericEvent)
            {
                ...
                Microsoft.SPOT.Presentation.UIElement targetWindow = TouchCapture.Captured;
                if (targetWindow == null)
                    targetWindow = MfeWindowManager.Instance.GetPointerTarget(genericEvent.X, genericEvent.Y);

                _inputManager.GenericDevice.SetTarget(targetWindow != null ? targetWindow : MainWindow);

                ir = new RawGenericInputReport(null, genericEvent);
                dev = _inputManager.GenericDevice;
            }
            else
            {
                /// Unkown event.
            ...
            Dispatcher.BeginInvoke(_reportInputMethod, new InputReportArgs(dev, ir));
        ...
        public void Run(MfeWindow window)
        {
            VerifyAccess();
            ...
            MfeWindowManager.EnsureInstance();
            ...
            if (window != null)
            {
                ...
                if (window.Visibility != Visibility.Visible)
                    Dispatcher.BeginInvoke(new MfeDispatcherOperationCallback(ShowWindow), window);
            }
        ...
        public void Shutdown()
        {
            ...
            {
                IsShuttingDown = true;
                Dispatcher.BeginInvoke(new MfeDispatcherOperationCallback(ShutdownCallback), null);
            }
        }
        #endregion
        ...
        internal MfeWindowCollection WindowsInternal
        {
            get
            {
                lock (typeof(GlobalLock))
                {
                    if (_appWindowList == null)
                        _appWindowList = new MfeWindowCollection();
                    return _appWindowList;
                }
            }
        }
        internal MfeWindowCollection NonAppWindowsInternal
        {
            get
            {
                lock (typeof(GlobalLock))
                {
                    if (_nonAppWindowList == null)
                        _nonAppWindowList = new MfeWindowCollection();
